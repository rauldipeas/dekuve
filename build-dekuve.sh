...
SUDO_ASKPASS="$HOME/sudo_askpass" sudo -A apt install -y ./debian-archive-keyring*.deb ./live-build*.deb
SUDO_ASKPASS="$HOME/sudo_askpass" sudo -A rm -rfv /tmp/dekuve
mkdir -p /tmp/dekuve
cd /tmp/dekuve
lb config\
	--apt-indices true\
...
SUDO_ASKPASS="$HOME/sudo_askpass" sudo -A lb build 2>&1|tee /tmp/build-dekuve.log